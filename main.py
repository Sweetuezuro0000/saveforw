...
# in_memory=True अनिवार्य है ताकि Render पर session file lock न हो
...
async def start_web_server():
    ...
    await runner.setup()
    # Render का डिफ़ॉल्ट पोर्ट 10000 होता है
...
async def set_target_topic(client, message: Message):
    try:
        ...
        await message.reply(f"🎯 Target Saved! Chat: `{chat_id}` | Topic: `{topic_id}`")
    except IndexError:
        await message.reply("❌ Usage: `/settarget <Link>`")
...
async def rem_word(client, message: Message):
    try:
        ...
        await message.reply(f"✅ Word Removed: `{word}`")
    except IndexError:
...
async def replace_word(client, message: Message):
    try:
        ...
        await message.reply(f"✅ Replacement Saved: `{old_word}` ➔ `{new_word}`")
    except ValueError:
...
async def batch_process(client: Client, message: Message):
    ...
    if len(args) < 3: return await message.reply("❌ Usage: `/batch <link> <count>`")
    ...
    except Exception as e: return await status_msg.edit_text(f"❌ Session Error: `{e}`")
    ...
    for i in range(count):
        ...
        try:
            ...
            # 1. Direct Server Copy
            try:
                if msg.media:
                    await msg.copy(**kwargs, caption=caption)
                elif msg.text:
                    await user_app.send_message(**kwargs, text=caption)
                success += 1
            except (ChatForwardsRestricted, RPCError):
                # 2. Restricted Fallback
                file_path = None
                try:
                    if msg.media:
                        ...
                        if file_path and os.path.exists(file_path):
                            if msg.video:
                                await bot.send_video(video=file_path, duration=msg.video.duration or 0, width=msg.video.width or 0, height=msg.video.height or 0, supports_streaming=True, caption=caption, **kwargs)
                            elif msg.photo:
                                await bot.send_photo(photo=file_path, caption=caption, **kwargs)
                            elif msg.document:
                                await bot.send_document(document=file_path, caption=caption, **kwargs)
                            elif msg.audio:
                                await bot.send_audio(audio=file_path, duration=msg.audio.duration or 0, caption=caption, **kwargs)
                            success += 1
                    elif msg.text:
