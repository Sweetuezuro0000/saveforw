...
# 🔹 Owner ID: Render Environment Variables में OWNER_ID नाम का Variable जोड़ें (उदा. 123456789)
OWNER_ID = int(os.environ.get("OWNER_ID", "0"))

# Owner-Only Filter
owner_filter = filters.user(OWNER_ID) if OWNER_ID != 0 else filters.all
...
def parse_target_link(link: str):
    ...
    m_priv_topic = re.match(r"https?://t\.me/c/(\d+)/(\d+)", link)
    if m_priv_topic: return int("-100" + m_priv_topic.group(1)), int(m_priv_topic.group(2))
    m_std = re.match(r"https?://t\.me/c/(\d+)", link)
...
async def set_target_topic(client, message: Message):
    try:
        ...
        await message.reply(f"🎯 Target Saved!\nChat ID: `{chat_id}`\nTopic ID: `{topic_id}`", parse_mode=enums.ParseMode.DISABLED)
    except IndexError:
...
async def del_target_topic(client, message: Message):
    ...
    await message.reply("🗑️ Target Reset to DM (Personal Chat)!")

@bot.on_message(filters.command("setcaption") & owner_filter)
...
@bot.on_message(filters.command("replace") & owner_filter)
...
# ----------------- BATCH LOGIC -----------------
@bot.on_message(filters.command("batch") & owner_filter)
async def batch_process(client: Client, message: Message):
    if not user_data["session"]: 
        return await message.reply("❌ Set session first via `/setsession`!", parse_mode=enums.ParseMode.DISABLED)
    
    args = message.text.split()
    if len(args) < 3: 
        return await message.reply("❌ Usage: `/batch [link] [count]`", parse_mode=enums.ParseMode.DISABLED)
    
    start_link, count = args[1], int(args[2])
    chat_id, start_msg_id = parse_source_link(start_link)
    if not chat_id or not start_msg_id: 
        return await message.reply("❌ Invalid Source Link!", parse_mode=enums.ParseMode.DISABLED)

    status_msg = await message.reply("⏳ Starting User Session & Caching Channels...")
    user_app = Client("UserSession", api_id=API_ID, api_hash=API_HASH, session_string=user_data["session"], in_memory=True)
    
    try:
        await user_app.start()
        # Private Channel Peer Cache Resolution
        try:
        ...
        return await status_msg.edit_text(f"❌ Channel Access Error: {e}\n👉 Make sure your Session Account is joined in this channel!", parse_mode=enums.ParseMode.DISABLED)

    dest_chat = user_data["target_chat"] or message.chat.id
    dest_topic = user_data["target_topic"] if user_data["target_chat"] else None

    # Resolve destination access hashes
    if user_data["target_chat"]:
        try: await user_app.get_chat(dest_chat)
        except Exception: pass
        try: await bot.get_chat(dest_chat)
        except Exception: pass
    ...
    for i in range(count):
        ...
        try:
            ...
            # Fallback for Restricted / Forward-Off Content
            if not copied:
                file_path = None
                try:
                    if msg.media:
                        ...
                        if file_path and os.path.exists(file_path):
                            ...
                            # Fallback upload via User Session if Bot lacks permission
                            except Exception:
                                if msg.video:
                                    await user_app.send_video(video=file_path, caption=caption, parse_mode=enums.ParseMode.DISABLED, **kwargs)
                                elif msg.photo:
                                    await user_app.send_photo(photo=file_path, caption=caption, parse_mode=enums.ParseMode.DISABLED, **kwargs)
                                elif msg.document:
                                    await user_app.send_document(document=file_path, caption=caption, parse_mode=enums.ParseMode.DISABLED, **kwargs)
                                elif msg.audio:
                                    await user_app.send_audio(audio=file_path, caption=caption, parse_mode=enums.ParseMode.DISABLED, **kwargs)
                            success += 1
                    elif msg.text:
                        await bot.send_message(text=caption, parse_mode=enums.ParseMode.DISABLED, **kwargs)
                        success += 1
                except Exception as ex:
                    print(f"Download/Upload Exception: {ex}")
                    failed += 1
                finally:
                    if file_path and os.path.exists(file_path):
                        os.remove(file_path)
            ...
            print(f"General Error on msg {msg_id}: {e}")
            failed += 1
